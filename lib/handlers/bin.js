...
module.exports = Observable.extend({
  ...
  apiCreateBin: function (req, res, next) {
    ...
    if (bin.html) {
      bin.html = beautify_html(bin.html, {"indent_size": 2, "wrap_line_length": 80});
    }
    if (bin.css) {
      bin.css = beautify_css(bin.css, {"indent_size": 2, "wrap_line_length": 80});
    }
    if (bin.javascript) {
      bin.javascript = beautify_js(bin.javascript, {"indent_size": 2, "wrap_line_length": 80});
    }
